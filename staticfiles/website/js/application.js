...
// }
    const wsScheme = window.location.protocol === "https:" ? "wss://" : "ws://";
        const namespace = "/poker/texas-holdem"; // Your Flask-SocketIO namespace
        const socketUrl = wsScheme + location.host + namespace;

        // Initialize Socket.IO connection
        PyPoker.socket = io.connect(socketUrl);
        ...
        PyPoker.socket.on("server_message", function(data) {
            console.log(data);

            switch (data.message_type) {
                case 'ping':
                    PyPoker.socket.emit("client_message", {'message_type': 'pong'});
                    break;
                case 'connect':
                    PyPoker.onConnect(data);
                    break;
        ...
        // Event listener for cards change
        $('#cards-change-cmd').click(function() {
            let discards = [];
            $('#current-player .card.selected').each(function() {
                discards.push($(this).data('key'));
            });
            PyPoker.socket.emit("client_message", {
                'message_type': 'cards-change',
                'cards': discards
            });
            PyPoker.Player.setCardsChangeMode(false);
        ...
        // Event listener for fold action
        $('#fold-cmd').click(function() {
            PyPoker.socket.emit("client_message", {
                'message_type': 'bet',
                'bet': -1
        ...
        // Event listener for no-bet action
        $('#no-bet-cmd').click(function() {
            PyPoker.socket.emit("client_message", {
                'message_type': 'bet',
                'bet': 0
    ...
    onConnect: function(message) {
        PyPoker.Logger.log("Connection established with poker server: " + message.server_id);
        $('#current-player').attr('data-player-id', message.player.id);
    },

    onDisconnect: function(message) {
        PyPoker.Logger.log("Disconnected from the poker server.");
    },
