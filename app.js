...
app.get('/table-data/:tableId', function (req, res) {
    ...
    // Check if the tableId is valid and exists in the tables array
    if (typeof tableId !== 'undefined' && tables[tableId]) {
        // Send the public data of the table
        res.send({ table: tables[tableId].public });
    } else {
        // Send an error response if the table doesn't exist
        res.status(404).send({ error: `Table with ID ${tableId} not found` });
    }
});
