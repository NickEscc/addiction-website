...
class GamePotsTest(unittest.TestCase):
    def test_add_bets_with_one_round(self):
        ...
        self.assertEqual(1500, game_pots[0].money)
        self.assertEqual([player1, player3, player4], game_pots[0].players)

        self.assertEqual(600, game_pots[1].money)
        self.assertEqual([player3, player4], game_pots[1].players)
    ...
    def test_add_bets_with_two_rounds(self):
        ...
        self.assertEqual(400, game_pots[0].money)
        self.assertEqual([player1, player2, player3, player4], game_pots[0].players)

        self.assertEqual(450, game_pots[1].money)
        self.assertEqual([player2, player3, player4], game_pots[1].players)

        self.assertEqual(100, game_pots[2].money)
        self.assertEqual([player2, player3, player4], game_pots[1].players)
    ...
    def test_add_bets_with_folders(self):
        ...
        self.assertEqual(700, game_pots[0].money)
    ...
    def test_add_bets_with_folders_two_rounds(self):
        ...
        self.assertEqual(1, len(game_pots))
        self.assertEqual(950.0, game_pots[0].money)
        self.assertEqual([player3, player4], game_pots[0].players)
    ...
    def test_add_bets_with_folders_two_rounds_all_in_players(self):
        ...
        self.assertEqual(300, game_pots[1].money)
        ...
        self.assertEqual(550, game_pots[1].money)
...
class GameScoresTest(unittest.TestCase):
    ...
    def test_assign_and_get_score(self):
        ...
        scores = GameScores(self.ScoreDetectorMock())
        scores.assign_cards("player-1", cards)
        self.assertEqual(123, scores.player_score("player-1").category)
        self.assertEqual(["5", "4", "3", "2", "1"], scores.player_score("player-1").cards)
...
class GameHoldemWinnerDetectorIntegrationTest(unittest.TestCase):
    def test_get_winners(self):
        ...
        self.assertEqual(4, len(game_pots))
...
class GameBetRounderTest(unittest.TestCase):
    def test_bet_round_everyone_fold(self):
        ...
        self.assertEqual("player-2", best_player.id)
    ...
    def test_bet_round_everyone_check(self):
        ...
        self.assertEqual("player-3", best_player.id)
    ...
    def test_bet_round_1(self):
        ...
        def bet_function_mock(player, min_bet, max_bet, bets):
            ...
            if bet_calls["counter"] == 1:
                self.assertEqual("player-2", player.id)
                self.assertEqual((0.0, 1000.0), (min_bet, max_bet))
                self.assertEqual({"player-1": 0.0, "player-2": 0.0, "player-3": 0.0, "player-4": 0.0}, bets)
                return 0.0
            elif bet_calls["counter"] == 2:
                self.assertEqual("player-3", player.id)
                self.assertEqual((0.0, 1000.0), (min_bet, max_bet))
                self.assertEqual({"player-1": 0.0, "player-2": 0.0, "player-3": 0.0, "player-4": 0.0}, bets)
                return -1
            elif bet_calls["counter"] == 3:
                self.assertEqual("player-4", player.id)
                self.assertEqual((0.0, 500.0), (min_bet, max_bet))
                self.assertEqual({"player-1": 0.0, "player-2": 0.0, "player-3": 0.0, "player-4": 0.0}, bets)
                return 500.0
            elif bet_calls["counter"] == 4:
                self.assertEqual("player-1", player.id)
                self.assertEqual((500.0, 600.0), (min_bet, max_bet))
                self.assertEqual({"player-1": 0.0, "player-2": 0.0, "player-3": 0.0, "player-4": 500.0}, bets)
                return 500.0
            elif bet_calls["counter"] == 5:
                self.assertEqual("player-2", player.id)
                self.assertEqual((500.0, 600.0), (min_bet, max_bet))
                self.assertEqual({"player-1": 500.0, "player-2": 0.0, "player-3": 0.0, "player-4": 500.0}, bets)
                return 600.0
            elif bet_calls["counter"] == 6:
                self.assertEqual("player-1", player.id)
                self.assertEqual((100.0, 100.0), (min_bet, max_bet))
                self.assertEqual({"player-1": 500.0, "player-2": 600.0, "player-3": 0.0, "player-4": 500.0}, bets)
                return 100.0

        bets = {}
        best_player = bet_rounder.bet_round("player-2", bets, bet_function_mock)

        self.assertEqual(6, bet_calls["counter"])
        ...
        self.assertEqual({"player-1": 600.0, "player-2": 600.0, "player-3": 0.0, "player-4": 500.0}, bets)
    ...
    def test_bet_round_2(self):
        ...
        def bet_function_mock(player, min_bet, max_bet, bets):
            ...
            if bet_calls["counter"] == 3:
                self.assertEqual("player-4", player.id)
                self.assertEqual((200.0, 1000.0), (min_bet, max_bet))
                self.assertEqual({"player-1": 0.0, "player-2": 0.0, "player-3": 200.0, "player-4": 0.0}, bets)
                return 600.0
            if bet_calls["counter"] == 4:
                self.assertEqual("player-1", player.id)
                self.assertEqual((500.0, 500.0), (min_bet, max_bet))
                self.assertEqual({"player-1": 0.0, "player-2": 0.0, "player-3": 200.0, "player-4": 600.0}, bets)
                return 500.0
            if bet_calls["counter"] == 5:
                self.assertEqual("player-3", player.id)
                self.assertEqual((400.0, 800.0), (min_bet, max_bet))
                self.assertEqual({"player-1": 500.0, "player-2": 0.0, "player-3": 200.0, "player-4": 600.0}, bets)
                return 800.0
            if bet_calls["counter"] == 6:
                self.assertEqual("player-4", player.id)
                self.assertEqual((400.0, 400.0), (min_bet, max_bet))
                self.assertEqual({"player-1": 500.0, "player-2": 0.0, "player-3": 1000.0, "player-4": 600.0}, bets)
                return -1
    ...
    def test_bet_round_small_and_big_blind(self):
        ...
        def bet_function_mock(player, min_bet, max_bet, bets):
            ...
            if bet_calls["counter"] == 1:
                self.assertEqual("player-1", player.id)
                self.assertEqual((200.0, 1000.0), (min_bet, max_bet))
                self.assertEqual({"player-1": 0.0, "player-2": 0.0, "player-3": 100.0, "player-4": 200.0}, bets)
                return -1
            if bet_calls["counter"] == 2:
                self.assertEqual("player-2", player.id)
                self.assertEqual((200.0, 1000.0), (min_bet, max_bet))
            ...
            if bet_calls["counter"] == 3:
                self.assertEqual("player-3", player.id)
                self.assertEqual((100.0, 900.0), (min_bet, max_bet))
                self.assertEqual({"player-1": 0.0, "player-2": 200.0, "player-3": 100.0, "player-4": 200.0}, bets)
                return 100.0
            if bet_calls["counter"] == 4:
                self.assertEqual("player-4", player.id)
                self.assertEqual((0.0, 800.0), (min_bet, max_bet))
                self.assertEqual({"player-1": 0.0, "player-2": 200.0, "player-3": 200.0, "player-4": 200.0}, bets)
                return 200.0
            if bet_calls["counter"] == 5:
                self.assertEqual("player-2", player.id)
                self.assertEqual((200.0, 800.0), (min_bet, max_bet))
                self.assertEqual({"player-1": 0.0, "player-2": 200.0, "player-3": 200.0, "player-4": 400.0}, bets)
                return -1
            if bet_calls["counter"] == 6:
                self.assertEqual("player-3", player.id)
                self.assertEqual((200.0, 800.0), (min_bet, max_bet))
            ...
            if bet_calls["counter"] == 7:
                self.assertEqual("player-4", player.id)
                self.assertEqual((200.0, 600.0), (min_bet, max_bet))
                self.assertEqual({"player-1": 0.0, "player-2": 200.0, "player-3": 600.0, "player-4": 400.0}, bets)
                return 400.0
            if bet_calls["counter"] == 8:
                self.assertEqual("player-3", player.id)
                self.assertEqual((200.0, 400.0), (min_bet, max_bet))
                self.assertEqual({"player-1": 0.0, "player-2": 200.0, "player-3": 600.0, "player-4": 800.0}, bets)
                return 400.0
            if bet_calls["counter"] == 9:
                self.assertEqual("player-4", player.id)
                self.assertEqual((200.0, 200.0), (min_bet, max_bet))
                self.assertEqual({"player-1": 0.0, "player-2": 200.0, "player-3": 1000.0, "player-4": 800.0}, bets)
                return 200.0
        ...
        self.assertEqual(9, bet_calls["counter"])

        self.assertEqual(1000.0, player1.money)
        self.assertFalse(game_players.is_active("player-1"))

        self.assertEqual(800.0, player2.money)
        self.assertFalse(game_players.is_active("player-2"))
        ...
        self.assertEqual("player-3", best_player.id)
        self.assertEqual({"player-1": 0.0, "player-2": 200.0, "player-3": 1000.0, "player-4": 1000.0}, bets)
    ...
    def test_bet_round_one_player_only(self):
        ...
        self.assertEqual(0, bet_calls["counter"])

        self.assertEqual("player-2", best_player.id)
        self.assertEqual({"player-2": 0.0}, bets)
